...
public class ExcelSheet
{
    ...
    {
        IEnumerable<ExcelRow> rows = ExcelReader.Read(srcSheet);
        ExcelSheet sheet = CreateSheet(rows, config);
    ...
    {
        IEnumerable<ExcelRow> rows = CsvReader.Read(path);
        ExcelSheet sheet = CreateSheet(rows, config);
        sheet.Name = "csv";
    ...
    {
        IEnumerable<ExcelRow> rows = TsvReader.Read(path);
        ExcelSheet sheet = CreateSheet(rows, config);
        sheet.Name = "tsv";
        return sheet;
    }
    ...
    {
        ExcelSheet sheet = CreateSheet(rows);
